...
function undo_code() {
  ...
  exec(' node .electron-vite/trimer.js  type=undo files=src/main/services/ipcMain.ts ', (error, stdout, stderr) => {
    if (error) {
...
function startElectron() {
  var args = ['--inspect=5859', join(__dirname, '../dist/electron/main/main.js')]
...
function greeting() {
  ...
  console.log(
    chalk.blue(`${config.dev.chineseLog ? '  准备启动...' : '  getting ready...'}`) + '\n'
  )
}
